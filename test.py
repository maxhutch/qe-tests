...
# define test output
def compare_vals(name, oval, rval, tol = 0.001):
  if (rval == 0 and oval == 0) or (rval != 0  and abs((oval - rval)/rval) < tol):
    print(" passed  %-14s % 10.6e vs % 10.6e" % (name, oval, rval))
  else:
    print(" FAILED  %-14s % 10.6e vs % 10.6e" % (name, oval, rval))
